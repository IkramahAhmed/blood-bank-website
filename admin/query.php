...
function clickme(query_id) {
    if (confirm("Do you really want to mark this as Read?")) {
        $.ajax({
            url: "query.php",  // Same file, so no need for a separate update_query.php
            type: "POST",
            data: { id: query_id },
            success: function(response) {
                if (response.trim() === "success") {  
                    alert("Query marked as Read.");
                    document.getElementById("status_" + query_id).innerHTML = "Read"; // Update status instantly
                } else {
...
<?php } else {?>
  ...
  <td>
    <a href="#" onclick="clickme(<?php echo $row['query_id']; ?>)">
        <b id="status_<?php echo $row['query_id']; ?>">Pending</b>
    </a>
</td>
